...
@Composable
fun HistoryContent(
    currencyName: String, historyData: List<History> = listOf(), modifier: Modifier = Modifier
) {
    Column(
        modifier = modifier
            .fillMaxSize()
        ...
        val entryModel = entryModelOf(historyData.map {
            FloatEntry(
                it.date.toLocalDate().monthValue.toFloat(), it.value
            )
        })
        Chart(
...
fun PreviewHistoryContent() {
    DollarStationTheme {
        HistoryContent(
            currencyName = "Turista", historyData = FAKE_DATA
        )
    }
}
